...
import os
import random
from typing import OrderedDict, Dict, Any, List
...
def get_answer_character(row: pd.DataFrame, name: str) -> pd.DataFrame:
    if len(row[name]) > 1:
        return max(row[name])
    elif len(row[name]) == 1:
        return row[name][0]
...
def remove_examples_longer_than_threshold(
    ...
    if (
        ...
        dataset = dataset.apply(tokenize_context, args=(doc_stride,), axis=1)
